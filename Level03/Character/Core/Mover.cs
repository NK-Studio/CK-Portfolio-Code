...
namespace Character.Core
{
    ...
    public class Mover : MonoBehaviour
    {
        ...
        private float _stepHeightRatio = 0.25f;

        [SerializeField, Header("콜라이더 옵션 :")] private float _colliderHeight = 2f;
        ...
        //Sensor variables;
        [SerializeField, Header("센서 옵션 :")] public Sensor.ECastType SensorType = Sensor.ECastType.Raycast;
        ...
        private int _sensorArrayRows = 1;

        [SerializeField] [Range(3, 10)] private int _sensorArrayRayCount = 6;
        [SerializeField] private bool _sensorArrayRowsAreOffset;
        ...
        private void OnValidate()
        {
            ...
            if (SensorType == Sensor.ECastType.RaycastArray)
                RaycastArrayPreviewPositions =
                    Sensor.GetRaycastStartPositions(_sensorArrayRows, _sensorArrayRayCount, _sensorArrayRowsAreOffset, 1f);
        }
        ...
        public void RecalculateColliderDimensions()
        {
            ...
            if (_boxCollider)
            {
                ...
                size.x = _colliderThickness;
                size.z = _colliderThickness;
        ...
        private void RecalibrateSensor()
        {
            ...
            //센서 반경 폭을 계산하십시오.
            float radius = _colliderThickness / 2f * SensorRadiusModifier;
            ...
            //센서 길이를 계산하고 설정합니다.
            float length = 0f;
            length += _colliderHeight * (1f - _stepHeightRatio) * 0.5f;
            length += _colliderHeight * _stepHeightRatio;
        ...
        private void Check()
        {
            ...
            if (_isUsingExtendedSensorRange)
                _sensor.CastLength = _baseSensorRange + (_colliderHeight * _tr.localScale.x) * _stepHeightRatio;
            else
            ...
            Vector3 localScale = _tr.localScale;
            float upperLimit = _colliderHeight * localScale.x * (1f - _stepHeightRatio) * 0.5f;
            float middle = upperLimit + _colliderHeight * localScale.x * _stepHeightRatio;
        ...
        {
            if (_colliderHeight == newColliderHeight)
                return;
        ...
        public float GetColliderHeight() => _colliderHeight;
        ...
        {
            if (_colliderThickness == newColliderThickness)
                return;
