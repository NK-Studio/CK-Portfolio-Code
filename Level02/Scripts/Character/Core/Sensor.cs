...
namespace Character.Core
{
	...
	public class Sensor {
		...
		private void CastRayArray(Vector3 origin, Vector3 direction)
		{
			...
			{
				//광선 시작 위치 계산
				rayStartPosition = origin + _tr.TransformDirection(_raycastArrayStartPositions[i]);

				if(Physics.Raycast(rayStartPosition, rayDirection, out RaycastHit hit, castLength, layermask, QueryTriggerInteraction.Ignore))
				{
					if(isInDebugMode)
						DebugX.DrawRay(hit.point, hit.normal, Color.red, Time.fixedDeltaTime * 1.01f);
		...
		public void DrawDebug()
		{
			if(_hasDetectedHit && isInDebugMode)
			{
				...
				DebugX.DrawRay(_hitPosition, _hitNormal, Color.red, Time.deltaTime);
				DebugX.DrawLine(_hitPosition + Vector3.up * markerSize, _hitPosition - Vector3.up * markerSize, Color.green, Time.deltaTime);
				DebugX.DrawLine(_hitPosition + Vector3.right * markerSize, _hitPosition - Vector3.right * markerSize, Color.green, Time.deltaTime);
				DebugX.DrawLine(_hitPosition + Vector3.forward * markerSize, _hitPosition - Vector3.forward * markerSize, Color.green, Time.deltaTime);
			}
		}
