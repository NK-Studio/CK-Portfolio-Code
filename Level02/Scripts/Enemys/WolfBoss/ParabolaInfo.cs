...
/// 포물선 정보를 담는 구조체입니다. 한 번 생성하면 상수 시간 안에 결정됩니다.
/// 수식은 https://www.desmos.com/calculator/wejssotbxw?lang=ko를 참조하세요.
public readonly struct ParabolaInfo {

    public readonly Vector3 Start;
    public readonly Vector3 End;
    public readonly float A; // a
    public readonly float P; // p
    ...
    public ParabolaInfo(Vector3 start, Vector3 end, float highest) {
        Start = start;
        End = end;
        ...
        if (x1 == 0) {
            ...
            float k = highest;
            float m = y1 / x1;
            float a = A = (y1 - 2 * (k + Mathf.Sqrt(k * (k - y1)))) / (x1 * x1);
            P = x1 - m / a;
            Valid = true;
        }
    }
    ...
    /// <param name="distanceFromOrigin">수식에서의 l값입니다.</param>
    /// <param name="distanceX">수식에서의 q값입니다.</param>
    public ParabolaInfo(Vector3 start, Vector3 end, float distanceFromOrigin, float distanceX) {
        Start = start;
        End = end;
        Vector3 between = end - start;
        Direction = between.normalized;
        ...
        if (x1 == 0) {
            ...
            float k = Mathf.Max(0, y1) + distanceFromOrigin / (Mathf.Abs(y1) - distanceX);
            float m = y1 / x1;
            float a = A = (y1 - 2 * (k + Mathf.Sqrt(k * (k - y1)))) / (x1 * x1);
            P = x1 - m / a;
            Valid = true;
